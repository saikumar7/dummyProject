(function() {
  ...
  function searchByName() {
    ...
    /** @ngInject */
    function searchByNameController(getChildrenFactory,$scope,$location, $http ) {
      var vm = this;
      ...
      vm.search = function () {
          ...
          $http.get('http://10.15.66.88/CCWISRest/Placement/OptimizePlacement', {params:{'ChildId': $scope.childIdArray[0], 'b': $scope.boysAgeArray.toString(), 'g': $scope.girlsAgeArray.toString(), 'ib': $scope.infantBoys, 'ig': $scope.infantGirls}}).
          then(function(response){
      ...
      vm.groupToPages = function () {
        ...
        for (var i = 0; i <  vm.placements.length; i++) {
            if (i % $scope.itemsPerPage === 0) {
                $scope.pagedItems[Math.floor(i / $scope.itemsPerPage)] = [vm.placements[i]];
            } else {
                $scope.pagedItems[Math.floor(i / $scope.itemsPerPage)].push( vm.placements[i]);
            }
            }
        };
